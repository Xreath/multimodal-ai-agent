...
Görüntüyü pipeline'a hazırlamak için preprocessing işlemleri.
"""
...
class ImagePreprocessor:
    """OpenCV tabanlı görüntü ön-işleme modülü."""

    @staticmethod
    def load_image(path: str) -> np.ndarray:
        """Görüntüyü diskten yükle."""
    ...
    def resize(image: np.ndarray, max_size: int = 1280) -> np.ndarray:
        """
        En-boy oranını koruyarak maksimum boyuta resize et.
        YOLO için genelde 640 veya 1280 kullanılır.
        """
        h, w = image.shape[:2]
        if max(h, w) <= max_size:
            return image
    ...
    def crop_region(image: np.ndarray, bbox: list) -> np.ndarray:
        """bbox [x1, y1, x2, y2] ile bölge kırp."""
    ...
    def get_image_info(image: np.ndarray) -> dict:
        """Görüntü hakkında temel bilgileri döner."""
        h, w = image.shape[:2]
