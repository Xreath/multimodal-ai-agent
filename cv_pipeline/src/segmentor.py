...
Extracts pixel-level masks of objects.
"""
...
class InstanceSegmentor:
    """YOLOv8-seg based instance segmentation module."""

    def __init__(self, model_name: str = "yolov8n-seg.pt", confidence: float = 0.25):
        """
        Args:
            model_name: YOLOv8-seg model file
    ...
    def segment(self, image: np.ndarray) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of segments: [{"label": str, "confidence": float, "bbox": [x1, y1, x2, y2], "mask_base64": str, "area_pixels": int}]
        """
        ...
        for result in results:
            ...
            for i, (box, mask) in enumerate(zip(boxes, masks)):
                ...
                # Get mask as binary numpy array
    ...
    def segment_and_draw(self, image: np.ndarray) -> tuple[np.ndarray, List[Dict]]:
        """Perform segmentation and draw overlay on the image."""
        results = self.model(image, conf=self.confidence, verbose=False)
