"""
Prompt Engine — CV pipeline JSON çıktısını LLM prompt'una optimal şekilde dönüştürür.

3 Strateji:
1. Direct Injection — CV JSON'ı system prompt'a göm
2. Few-Shot — Örnek input/output çiftleri ekle
3. Chain-of-Thought (CoT) — Adım adım düşünme talimatı ver

Mülakat notu:
- Prompt engineering, Senior AI Engineer rolünün en kritik yetkinliklerinden biri
- System prompt tasarımı LLM davranışını şekillendirir
- Few-shot örnekler LLM'i "kalibre" eder — özellikle format tutarlılığı için
- CoT, karmaşık reasoning görevlerinde doğruluğu artırır (Wei et al., 2022)
"""
...
class PromptEngine:
    """CV pipeline çıktısını LLM prompt'una dönüştürür."""

    @staticmethod
    def format_cv_context(cv_result: dict, include_raw_coords: bool = True) -> str:
        """
        CV pipeline JSON'ını LLM-dostu text formatına çevir.

        Neden JSON'ı direkt vermiyoruz?
        → Verebiliriz (ve JSON mode'da veririz), ama insana okunur text
          formatı LLM'in reasoning kalitesini artırır.
          Ayrıca token tasarrufu sağlar — sadece gerekli alanları dahil edebiliriz.
        """
        lines = []
    ...
    def build_direct_prompt(
        ...
        """
        Direct Injection stratejisi.
        CV context'i system prompt'a gömüp, user sorusunu ayrı gönderir.

        Bu en basit ama etkili yöntem:
        - System prompt → rol + kurallar
        - User message → CV data + soru
        """
    ...
    def build_few_shot_prompt(
        ...
        """
        Few-Shot stratejisi.
        Örnek bir CV çıktısı + ideal cevap çifti ekleyerek LLM'i "kalibre" eder.

        Mülakat notu:
        - Few-shot, LLM'in output formatını tutarlı tutmasını sağlar
        - 1-3 örnek genelde yeterli (daha fazla → token israfı)
        - Örnekler gerçekçi olmalı — sahte data LLM'i yanıltır
        """
        cv_context = PromptEngine.format_cv_context(cv_result)

        # Örnek CV data + ideal cevap
...
=== OCR Text Regions (1) ===
  1. "DANGER ZONE" (confidence: 94.2%)"""
    ...
    def build_cot_prompt(
        ...
        """
        Chain-of-Thought (CoT) stratejisi.
        LLM'e "adım adım düşün" talimatı vererek reasoning kalitesini artırır.

        Mülakat notu:
        - CoT, Wei et al. (2022) tarafından önerildi
        - Karmaşık sorularda ~15-20% doğruluk artışı sağlar
        - Dezavantaj: daha fazla token kullanır → daha yavaş ve pahalı
        - "Let's think step by step" bile işe yarar ama yapısal talimat daha iyi
        """
        cv_context = PromptEngine.format_cv_context(cv_result)
...
Show your reasoning in the reasoning_steps field."""
    ...
    def build_comparison_prompt(
        ...
        """
        CV pipeline çıktısı ile multi-modal LLM çıktısını karşılaştırma prompt'u.
        GPT-4V/Gemini Vision'ın direkt görüntü analizi vs bizim pipeline.
        """
        cv_context = PromptEngine.format_cv_context(cv_result)
    ...
    def build_tool_use_system_prompt(tools_schema: list[dict]) -> str:
        """
        Tool use için system prompt oluştur.
        Bu prompt, LLM'e hangi tool'ları kullanabileceğini söyler.

        Not: OpenAI ve Anthropic'in native function calling'i varken
        neden manual tool prompt? → Eğitim amaçlı + Gemini'de
        function calling farklı çalışıyor, unified bir yaklaşım lazım.
        """
